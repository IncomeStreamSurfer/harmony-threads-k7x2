...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const SB_KEY = SERVICE_ROLE || (import.meta.env.PUBLIC_SUPABASE_ANON_KEY ?? process.env.PUBLIC_SUPABASE_ANON_KEY ?? "");
    if (SUPABASE_URL && SB_KEY) {
      const sb = createClient(SUPABASE_URL, SB_KEY, { auth: { persistSession: false } });
